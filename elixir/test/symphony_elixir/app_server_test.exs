defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server replaces invalid utf-8 in outbound turn payloads" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      ...
      issue = %Issue{
        ...
        identifier: "MT-UTF8",
        title: <<0xE5>>,
        description: "Ensure invalid UTF-8 is sanitized before JSON encoding",
        state: "In Progress",
        url: "https://example.org/issues/MT-UTF8",
        labels: ["backend"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Prompt " <> <<0xE5>>, issue)
      ...
      turn_start_payload =
        Enum.find_value(lines, fn line ->
          if String.starts_with?(line, "JSON:") do
            ...
            if payload["id"] == 3, do: payload
          end
        end)

      assert turn_start_payload["method"] == "turn/start"
      assert get_in(turn_start_payload, ["params", "title"]) == "MT-UTF8: ?"
      assert get_in(turn_start_payload, ["params", "input", Access.at(0), "text"]) == "Prompt ?"
    after
      File.rm_rf(test_root)
    end
  end
